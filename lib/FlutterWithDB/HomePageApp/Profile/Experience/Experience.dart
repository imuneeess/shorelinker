...
class Experience extends StatefulWidget {
  ...
  @override
  State<Experience> createState() => _FloatAction();

}
class _FloatAction extends State<Experience> {
  ...
  Future<void> _checkAndLoadProfileData() async {
    ...
    try {
      ...
      if (querySnapshot.docs.isNotEmpty) {
        ...
        if(userData!['Entreprise'] == null){
          print('Pret pour ajouter les nouvelle champs de user Auth!!');
        }
        else{
          controllerEntreprise.text = userData!['Entreprise'];
          controllerTitre.text = userData!['Titre'];
          controllerAnnee.text = userData!['Annee'];
          controllerMois.text = userData!['Mois'];
          controllerLieu.text = userData!['Lieu'];
          controllerDesc.text = userData!['Desc'];
        }
      } else {
  ...
  Future<void> _saveProfileData() async {
    ...
    try {
      ...
      await profiles.doc(widget.docid).update({
        'Entreprise' : controllerEntreprise.text,
        'Titre' : controllerTitre.text,
        'Annee' : controllerAnnee.text,
        'Mois' : controllerMois.text,
