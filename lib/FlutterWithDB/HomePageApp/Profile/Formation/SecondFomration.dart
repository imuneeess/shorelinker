...
class SecondFormation extends StatefulWidget {
  final String docid;
  const SecondFormation({super.key, required this.docid});

  @override
  State<SecondFormation> createState() => _FloatAction();

}
class _FloatAction extends State<SecondFormation> {
  ...
  Future<void> _checkAndLoadProfileData() async {
    ...
    try {
      ...
      if (querySnapshot.docs.isNotEmpty) {
        ...
        if(userData!['SecondUniversite'] == null){
          print('Pret pour ajouter les nouvelle champs de user Auth!!');
        }
        else{
          controllerUniversite.text = userData!['SecondUniversite'];
          controllerDiplome.text = userData!['SecondDiplome'];
          controllerDomaine.text = userData!['SecondDomaine'];
          controllerResultat.text = userData!['SecondResultat'];
          controllerAnneeD.text = userData!['SecondAnneeD'];
          controllerAnneeF.text = userData!['SecondAnneeF'];
        }
      } else {
  ...
  Future<void> _saveProfileData() async {
    ...
    try {
      ...
      await profiles.doc(widget.docid).update({
        'SecondUniversite' : controllerUniversite.text ,
        'SecondDiplome' : controllerDiplome.text,
        'SecondDomaine' : controllerDomaine.text,
        'SecondResultat' : controllerResultat.text,
        'SecondAnneeD' : controllerAnneeD.text,
        'SecondAnneeF' : controllerAnneeF.text,
      });
